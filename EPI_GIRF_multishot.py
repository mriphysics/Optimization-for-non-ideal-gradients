...
res            = 128
nseg           = 16  # epi segmentation
navg           = 1   # number of repeated epi images (mainly to see transient state effects)
TRfill         = 5   # seconds at the end of each shot

smax           = 180
gmax           = 72e-3

experiment_id = 'multishot_11R'
...
size_tmp = [res,res]
params_target = EPI2D_EC(res, nseg, navg, TRfill=TRfill)
...
if pulseq == 1:
    seq_full_copy = seq_full.clone()
    pulseq_write_EPG_3D_EC_EPI(seq_full_copy, 'multishot_11R1.seq', FOV_export, GIRF, plot_seq=True, smax=smax)
...
if DO_IMAGE_SIM:
    graph_unperturbed = mr0.compute_graph(seq_full, data, max_state_count=max_state_count, min_state_mag=min_state_mag)
    graph_perturbed = mr0.compute_graph(seq_full_perturbed, data, max_state_count=max_state_count, min_state_mag=min_state_mag)
    ...
    # reconstructions
    target_reco_full_unperturbed = reconstruct_EPI_fft_naive(seq_full,target_signal_full_unperturbed,size,nseg)
    target_reco_full_perturbed = reconstruct_EPI_fft_naive(seq_full_perturbed,target_signal_full_perturbed,size,nseg)
...
def calc_loss(gradm_all: torch.Tensor,
    ...
    if DO_IMAGE_OP:
        ...
        reco = sos(reconstruct_EPI_fft_naive(seq, signal, size, nseg))
    ...
    if (iteration == 1) or (iteration % 5000 == 0):
        ...
        shot1_end = int(len(waveform_x)/16)        
        plt.subplot(413)
...
size_tmp = [res,res]
params = EPI2D_EC(res, nseg, navg, TRfill=TRfill)
...
for restart in range(NRestarts):
    ...
    for i in range((restart + 1) * NIter):
        ...
        if i % 10 == 0:
            graph = mr0.compute_graph(seq_opt, data, max_state_count=max_state_count, min_state_mag=min_state_mag)
...
if pulseq == 1:
    ...
    seq_full_copy = seq.clone()  
    pulseq_write_EPG_3D_EC_EPI(seq_full_copy, 'multishot_11R2.seq', FOV_export, GIRF, plot_seq=False, smax=smax+1)